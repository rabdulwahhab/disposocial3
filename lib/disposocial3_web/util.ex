defmodule Disposocial3Web.Util do
  ...
  def display_relative_time_future(datetime) do
    ...
    cond do
      minutes_left > 0 and minutes_left > 60 ->
        "#{div(minutes_left, 60)} hour(s) and #{rem(minutes_left, 60)} minute(s) left"

      minutes_left > 0 ->
        "#{minutes_left} minute(s) left"

      true ->
        "#{abs(DateTime.diff(datetime, now, :second))} second(s) left"
    end
  end
  ...
  def display_relative_time_past(datetime) do
    ...
    cond do
      minutes_left < 0 and abs(minutes_left) > 60 ->
        "#{div(abs(minutes_left), 60)} hour(s) and #{rem(abs(minutes_left), 60)} minute(s) ago"

      minutes_left < 0 ->
        "#{abs(minutes_left)} minute(s) ago"

      true ->
        "#{abs(DateTime.diff(datetime, now, :second))} second(s) ago"
    end
  end

  def display_post_time(utc_dt, timezone) do
    datetime = DateTime.shift_zone!(utc_dt, timezone)
    display_post_time(datetime)
  end
  ...
  def display_death_datetime(utc_dt, timezone) do
    datetime = DateTime.shift_zone!(utc_dt, timezone)
    display_death_datetime(datetime)
  end
