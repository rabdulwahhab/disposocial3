...
alias Disposocial3.{Repo, Accounts.User, Accounts.Scope, Dispos, Posts}

future_death = DateTime.utc_now() |> DateTime.truncate(:second) |> DateTime.add(120000) # 600 secs == 10 min
# 39.1806976, -77.2440064
{:ok, user1} = Repo.insert(%User{id: 3, username: "Rayyan", email: "r@r.com", password: "testtesttest"})
{:ok, user2} = Repo.insert(%User{id: 2, username: "Man", email: "m@m.com", password: "testtesttest"})
scope1 = Scope.for_user(user1)
scope2 = Scope.for_user(user2)
{:ok, _} = Dispos.create_dispo(scope1, %{id: 5, name: "Dispo1", description: "I am a description", latitude: 39.002, longitude: -77.00, is_public: true, user_id: 1, death: future_death, location: "Germantown, MD, USA"})
...
{:ok, _} = Posts.create_post(scope1, %{body: "Just some text", user_id: 1, dispo_id: 1})
{:ok, _} = Posts.create_post(scope1, %{body: "Just some text", user_id: 1, dispo_id: 1})
{:ok, _} = Posts.create_post(scope1, %{body: "Just some text", user_id: 2, dispo_id: 1})
{:ok, _} = Posts.create_post(scope1, %{body: "Just some text", user_id: 2, dispo_id: 1})
{:ok, _} = Posts.create_post(scope1, %{body: "Just some text", user_id: 2, dispo_id: 1})
{:ok, _} = Posts.create_post(scope1, %{body: "Just some text", user_id: 2, dispo_id: 1})
{:ok, _} = Posts.create_post(scope1, %{body: "Just some text", user_id: 2, dispo_id: 1})
